...
const AutoExpandingTextarea: React.FC<AutoExpandingTextareaProps> = ({
  ...
  return (
    <textarea
      ...
      className="w-full px-3 py-2 border border-slate-200 rounded bg-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none overflow-y-auto min-h-[58px] max-h-[124px]"
      style={{ height: "auto" }}
    />
  );
};
    ...
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest text-slate-800">Questions List</h2>
        </div>
      ...
      <div className="space-y-4">
        {questions.length === 0 ? (
          ...
          questions.map((q, index) => (
            <div
              key={q.id}
              className="group border border-slate-200 hover:border-slate-300 rounded-lg p-4 bg-slate-50/50 hover:bg-white transition-all space-y-3"
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                ...
                <div className="sm:col-span-3">
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-200 rounded bg-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                  />
              ...
              <div className="pl-4 border-l-2 border-slate-200 space-y-2 mt-2">
                {q.subQuestions.map((sq) => (
                  <div key={sq.id} className="flex items-start space-x-2 bg-white p-2 rounded border border-slate-200">
                    <input
                      type="text"
                      value={sq.label}
                      onChange={(e) => updateSubLabel(q.id, sq.id, e.target.value)}
                      placeholder="label"
                      className="w-10 text-center px-1 py-1 border border-slate-200 rounded text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
      ...
      <button
        onClick={addQuestion}
        className="w-full flex items-center justify-center space-x-2 border-2 border-dashed border-slate-200 hover:border-blue-400 text-blue-600 hover:text-blue-800 py-3 rounded font-semibold text-sm transition-all bg-slate-50/50 hover:bg-blue-50/20 cursor-pointer"
      >
